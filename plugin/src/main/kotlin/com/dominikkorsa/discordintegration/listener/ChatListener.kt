...
class ChatListener(private val plugin: DiscordIntegration) : Listener {
    ...
    suspend fun onPlayerChat(event: AsyncPlayerChatEvent) {
        ...
        if (!plugin.configManager.chat.messages.enabled) {
            if (logCancelledEvents == ALL) {
                plugin.logger.warning(
                    "Message sent by player ${event.player.name} is ignored, because `chat.messages.enabled` is set to `false`"
                )
                plugin.logger.warning("You can disable this warning by setting `debug.log-cancelled-chat-events` to `auto` or `disable`")
            }
            return
        }
        ...
        plugin.webhooks.sendChatMessage(event.player, event.message)
    }
}
