...
class MinecraftFormatter(val plugin: DiscordIntegration) {
    suspend fun formatDiscordMessage(
        ...
        return template
            ...
            .replace("%content%", plugin.emojiFormatter.replaceEmojis(message.content.trimEnd()))
    }

    fun formatHelpHeader() = plugin.messageManager.commandsHelpHeader
        .replace("%plugin-version%", plugin.description.version)

    fun formatHelpCommand(command: String, code: String) = plugin.messageManager.commandsHelpCommand
